...
class FieldAwareFMRecommender(BaseRecommender):
    ...
    def __init__(self, URM_train, train_svm_file_path, approximate_recommender: BaseRecommender,
                 ...
                 max_items_to_predict=1000, model_filename="model.out",
    ...
    def recommend(self, user_id_array, cutoff=None, return_scores=False):
        ...
        if not os.path.isfile(recommendation_file):
            ...
            if self.ICM_train is not None:
                fm_matrix = add_ICM_info(
                    fm_matrix, self.ICM_train, self.URM_train.shape[0])
            ...
            write_data_to_xlearn_format(
                X=fm_matrix, y=labels, filepath=recommendation_file, fields=self.fields)
        ...
        ranking_list = items_to_recommend[np.arange(
            scores_batch.shape[0]), np.transpose(score_index_list)].T
